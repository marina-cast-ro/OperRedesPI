...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Simulacion de sensor de movimiento (sin Raspberry Pi)"
    )
    ...
    parser.add_argument(
        "--interval", type=int, default=10, help="Intervalo de envio en segundos"
    )
    parser.add_argument(
        "--motion-every",
        type=int,
        default=4,
        help="Cada cuantos segundos simular un movimiento",
    )
...
# Trunca/vacía el archivo al iniciar para evitar acumular ejecuciones anteriores
...
def onMotion():
    storage.storeData(
        f"[{datetime.now():%Y-%m-%d %H:%M:%S}] Detected movement (SIMULADO)", args.file
    )
